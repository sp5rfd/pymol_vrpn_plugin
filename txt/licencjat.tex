...
\\
\\
Kwaterniony mogą stanowic alternatywną wobec współrzędnych jednorodnych lub macierzy transformacji formę opisu rotacji. Dzięki temu możemy z powodzeniem przeprowadzać konwersję pomiędzy nimi a kwaternionami. 

Dzięki kwaternionom z powodzeniem można przedstawić rotację wokół arbitralnie wybranej osi obrotu. Załóżmy, że chcemy stworzyć kwaternion $q$ reprezentujący obrót o kąt $\alpha$ wokół wektora $\vec{v}=[v_x,v_y,v_z]$. Kwaternion taki tworzymy w następujący sposób
...
Kwaterniony mogą reprezentować nie tylko rotację (operację obrotu) ale także orientację. Wykorzystana w niniejszej pracy biblioteka VRPN wydaje informację na orientacji wskaźnika właśnie za pomocą kwaternionu. Pakiet PyMOL natomiast akceptuje jedynie \textit{zmiany orientacji} wobec swoich obiektów, czyli rotację. Aby prawidłowo przekonwertować ...
