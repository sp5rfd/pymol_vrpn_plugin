...
\\
\\
Współrzędne punktu $P=(x,y,z)$ po operacji skalowania o wektor współczynników $\vec{S}=[S_x,S_y,S_z]^T$ reprezentuje równanie:
...
$$
\\
Aby przedstawić operację skalowania we współrzędnych jednorodnych należy stworzyć nową macierz $\bar{S}$ o rozmiarze o 1 większym, niż macierz elementarna $\hat{S}$ oraz nowy wektor $P_2$ o długości o 1 większej niż $P$:
$$
\bar{S} = 
\begin{bmatrix}
S_x & 0 & 0 & 0 \\
...
Reprezentacja przy pomocy współrzędnych jednorodnych ponownie wymaga skonstruowania w odpowiedni sposób macierzy translacji poprzez wpisanie do niej w odpowiednim miejscu wektora translacji $\vec{T}$: 
...
W przypadku rotacji, poza współrzędnymi jednorodnymi należy tutaj rozpatrzyć także reprezentację za pomocą kwaternionów. 
Wygodnym sposobem reprezentacji rotacji są kwaterniony. Kwaterniony zostały wprowadzone przez Hamiltona, który poszukiwał wygodnego i sposobu reprezentacji obrotów.
Kwaterniony to czwórka liczb 
