...
W tym rozdziale zaprezentowano podstawowe twierdzenia oraz teorię leżącą u podstaw niniejszej pracy. Skupiono się głównie na zagadnieniach związanych z przekształceniami geometrycznymi, wykorzystanymi algorytmami oceny podobieństwa strukturalnego oraz metodami dopasowania lokalnego (uliniawiania) struktur chemicznych.
...
Skalowanie (inaczej mnożenie przez skalar) to operacja polegająca na mnożeniu wszystkich współrzędnych obiektu określonego w przestrzeni trójwymiarowej przez dodatnią liczbę rzeczywistą $C$ lub dodatnio określony wektor $\vec{C}$ będące współczynnikami skalowania. W ogólności możemy skalować wszystkie składowe wektora współrzędnych niezależnie przez stosowanie jako współczynników skalowania wektorów. Dla potrzeb niniejszej pracy wystarczy jednak, że wykonywane będzie skalowanie jako operacja mnożenia wektora współrzędnych przez skalar $C$.

Geometryczna intuicja stojąca za operacją skalowania polega na takim przekształceniu współrzędnych danego obiektu, że w zależności od wartości  współczynnika skalującego może zostać: proporcjonalnie zmniejszony gdy $C<1$, proporcjonalnie powiekszony gdy $C>1$ lub gdy $C=1$ pozostanie niezmieniony.

Dla $N$ elementowego wektora współrzędnych $\vec{p}$:
$$
\vec{p}=
\begin{pmatrix}
x_1 & y_1 & z_1 \\
...
Operacja przesunięcia wykorzystana jest w pracy w każdym przypadku zmiany położenia wskaźnika urządzenia haptycznego czy podczas wykonywania operacji superpozycji i dopasowania struktur. Dla wektora współrzędnych $\vec{p}$ operacja przesunięcia wygląda następująco:
...
\vec{p'}=
\begin{pmatrix}
x_1+t_x & Cy_1+t_y & Cz_1+t_z \\
Cx_2+t_x & Cy_2+t_y & Cz_2+t_z \\
\vdots & \vdots & \vdots \\
Cx_N+t_x & Cy_N+t_y & Cz_N+t_z 
\end{pmatrix}
$$
\subsection{Rotacje i kwaterniony}
todo: rotacje i kwaterniony

\section{Elementarne macierze przekształceń}
Współcześnie wszelkie operacje graficzne, a w szczególności tranformacje są wykonywane na milionach punktów (pikseli). Często także pojawia się sytuacja w której przekształcenia geometryczne są wykonywane jednocześnie. Można sobie wyobrazić sytuację w której obiekt przestrzenny należy przesunąć jednocześnie dokonując obrotu. 

Bezpośrednia implementacja powyższych transformacji byłaby dość kłopotliwa, gdyż każda operacja musiałaby zostać wykonana oddzielnie. Jednak przede wszystkim nie byłaby optymalna obliczeniowo. Do tego celu opracowano ujednolicone narzędzie, doskonale spełniające zadanie: \textit{macierze przekształceń}.

Macierze przekształceń powstały przede wszystkim z myślą o implementacji na współczesnych komputerach, które doskonale radzą sobie z obliczeniami macierzowymi. Istnieją dziesiątki rozmaitych bibliotek wspomagających tego typu obliczenia, ponadto operacje macierzowe można łatwo zrównoleglać i optymalizować.

Macierze przekształceń stanowią ujednolicony i łatwy sposób posługiwania się wszystkimi zdefiniowanymi transformacjami, a w szczególności: skalowaniem, translacją i rotacją. Każdą z powyższych operacji można opisać jako operacje macierzowe modyfikujące zbiór punktów w przestrzeni.
\\
...
\end{bmatrix}
$$
możemy zdefiniowac taką macierz $Trans(a,b,c)$:
$$
\vec{Trans(a,b,c)}=\begin{bmatrix}
a \\
b \\
c
\end{bmatrix}
...
\section{Współrzędne jednorodne i przekształcenia macierzowe}
...
W pierwszym kroku należy dokonać obliczenia centroidów ($Cp$ i $Cq$) obu struktur i dokonać przesunięcia struktury wzorca o wektor $T$ rozpięty pomiędzy tymi punktami tak, aby nałożyły się one na siebie. Do obliczenia centroidu można zastosować na przykład równanie na środek geometryczny: 
$$Cp = (Cp_x, Cp_y, Cp_z)$$
gdzie:
