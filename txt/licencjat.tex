...
Przekształcenia geometryczne w przestrzeni trójwymiarowej stanowią jedne z elementarnych operacji często używanych w niniejszej pracy. Do podstawowych przekształceń zaliczamy między innymi skalowanie, translacje i rotacje. Wszystkie te operacje można praktycznie zrealizować na kilka sposobów. Z racji tego, że przekształcenia geometryczne operują na milionach punktów (pikseli) w przestrzeni trójwymiarowej, to optymalne metody numeryczne je realizujące są priorytetem i muszą w minimalnym stopniu obciążać komputerowe jednostki obliczeniowe. 

Współpraca bibliotek programistycznych (pochodzących od różnych dostawców) może wymagać konwersji pomiędzy różnymi formatami zapisu transformacji. Szczególna uwagę należy zwrócić na konwersję orientacji wskaźnika z formatu zwracanego przez bibliotekę VRPN (kwaterniony) na format akceptowany przez PyMOL (macierze rotacji) i odwrotnie. Ten aspekt zostanie omówiony w dalszej części pracy.
...
$\vec{P'}$ jest wektorem współrzędnych punktu $[x',y',z']^T$ po dokonanych transformacjach

$\vec{P}$ jest wektorem współrzędnych punktu $[x,y,z]^T$ poddanego transformacjom
...
Z uwagi na konieczność dostosowania transformacji do wykonywania na współczesnych komputerach opracowano efektywne reprezentacje tych operacji. 
